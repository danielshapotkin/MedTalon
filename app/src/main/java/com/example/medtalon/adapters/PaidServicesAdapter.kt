...
class PaidServicesAdapter(context: Context, private val paidServices: List<PaidService>) :
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        getTalonButton.setOnClickListener{
            dataBase.setTalon(date = paidService.date, time = paidService.time, doctor = paidService.name, polyclinic = paidService.polyclinic){isSuccess, error->
            }
            Toast.makeText(context, "Талон успешно заказан", Toast.LENGTH_LONG)
        }
